...
expressions.icosahedron = {
  construct: function(options) {
    this.radius = options.radius || 1;
    this.geometry = new THREE.IcosahedronGeometry(this.radius, 0);
    addContainsPoint(this.geometry);
  },
  evaluate: function(point) {
    var local = this.local(point);
    return this.geometry.containsPoint(local);
  }
};

expressions.dodecahedron = {
  construct: function(options) {
    this.radius = options.radius || 1;
...
// Broken: it looks like some THREE.Geometries are not closed
expressions.torusknot = {
  construct: function(options) {
    this.radius = options.radius || 5;
    this.tube = options.tube || 1;
    this.radialSegments = options.radialSegments || 64;
    this.tubularSegments = options.tubularSegments || 8;
    this.geometry = new THREE.TorusKnotGeometry( this.radius, this.tube,
...
// pass any THREE Geometry
expressions.fromThreeGeometry = {
  construct: function(options) {
