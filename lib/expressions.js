...
exports.Expression.prototype.cartesian2spherical = function(point) {
  var d = Math.sqrt(point[0] * point[0] + point[1] * point[1] + point[2] * point[2]);
  return ([
    d,
    Math.atan2(point[1], point[0]),
    d ? Math.acos(point[2] / d) : 0
  ]);
};

exports.Expression.prototype.spherical2cartesian = function(spherical) {
...
expressions.torus = {
  ...
  evaluate: function(point) {
    ...
    if (local[2] * local[2] + Math.pow(Math.sqrt(
        local[0] * local[0] + local[1] * local[1]) - this.radius, 2) < Math.pow(this.tube/2, 2)) {
      return 255;
    } else {
      return 0;
    }
...
expressions.loxodrome = {
  ...
  evaluate: function(point) {
    var local = this.local(point);
    var spherical = this.cartesian2spherical(local);
